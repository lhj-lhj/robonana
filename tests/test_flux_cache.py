...
def test_select_current_future_latents_uses_horizon_and_clamps():
    ...
    current, future = select_current_future_latents(latents, current_index=3, chunk_horizon=4)
...
def test_select_current_future_latents_rejects_nonpositive_horizon():
    with pytest.raises(ValueError, match="positive"):
        select_current_future_latents(torch.zeros(2, 3, 4), current_index=0, chunk_horizon=0)
