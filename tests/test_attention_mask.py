import torch

from robonana.models.attention_mask import SegmentMap, build_attention_bias


def test_attention_layout_is_bidirectional_for_both_action_tracks():
    seg = SegmentMap.from_lengths(
        language=2,
        state=1,
        ref_image=2,
        pred_action=4,
        gt_action=4,
        horizon=1,
        future_state=1,
        value=1,
        future_image=3,
    )
    bias = build_attention_bias(seg, batch_size=1, dtype=torch.float32, device="cpu")[0, 0]

    assert torch.isfinite(bias[seg.pred_action, seg.pred_action]).all()
    assert torch.isfinite(bias[seg.gt_action, seg.gt_action]).all()
    assert torch.isneginf(bias[seg.pred_action, seg.gt_action]).all()
    assert torch.isneginf(bias[seg.gt_action, seg.pred_action]).all()
    assert torch.isneginf(bias[seg.future_image, seg.pred_action]).all()
    assert torch.isfinite(bias[seg.future_image, seg.gt_action]).all()


def test_padded_context_keys_are_blocked_without_all_masked_rows():
    seg = SegmentMap.from_lengths(
        language=3,
        state=1,
        ref_image=1,
        pred_action=1,
        gt_action=1,
        horizon=1,
        future_state=1,
        value=1,
        future_image=1,
    )
    context_mask = torch.tensor([[True, True, False]])
    bias = build_attention_bias(
        seg,
        batch_size=1,
        dtype=torch.float32,
        device="cpu",
        context_mask=context_mask,
    )[0, 0]
    padded = 2
    assert not torch.isneginf(bias[:, padded]).all().item()
    assert torch.isfinite(bias[padded, padded])
    valid_queries = torch.arange(seg.total_length) != padded
    assert torch.isneginf(bias[valid_queries, padded]).all()
